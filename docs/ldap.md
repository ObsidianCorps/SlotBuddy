...
can be used to test the LDAP integration of SlotBuddy.
...
## Enabling Integration in SlotBuddy

After making sure that the local OpenLDAP server works, SlotBuddy will be able to connect to it.

For this you will need to go to the Backend > Settings > Integrations > LDAP and enable the integration from there. 

This settings page also requires some configuration for SlotBuddy to be able to connect to the server and pull
...
If the filter was right, you will be getting LDAP entries displayed on screen that can be imported into SlotBuddy.
...
While SSO with LDAP on SlotBuddy, the username will need to be matched for the SSO to work. Also, the LDAP DN 
...
If LDAP is enabled, SlotBuddy will first try to check for the local credentials, but if this does not work, then 
