...
the CalDAV integration of SlotBuddy.
...
- The right time zone value needs to be selected for synchronization to match the information SlotBuddy sends.
...
After making sure that the local Baikal server works, SlotBuddy will be able to connect to it and sync the
appointment data.
...
While trying to enable the CalDAV sync from the SlotBuddy calendar page, use the following credentials after
